...
function crearElemento(i, title, autor) {
    ...
    a.href = "./Libro.html?number=" + i

    const div = document.createElement("div")
    div.className = "project-image-wrapper"

    const img = document.createElement("img")
    img.src = "./Libros_Images/" + i + ".jpg"
    img.loading = "lazy"
...
function crearEmpty() {
    const p = document.createElement("p")
    p.className = "regular-paragraph white"
    p.innerHTML = "No hay libros disponibles."
    return p
}
...
function addBooks(type) {
    ...
    fetch(name)
        ...
        .then((text) => {
            if (text == null || text == "") {
                document.getElementById("listaLibros").appendChild(crearEmpty())
            } else {
                const libros = text.split('\n')
                for (i in arrayRange(0, libros.length - 1, 1)) {
                    const data = libros[i].split("%");
                    document.getElementById("listaLibros").appendChild(crearElemento(i, data[0], data[1]))
                }
            }
        })
